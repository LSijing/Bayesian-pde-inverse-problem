...
class MyTestCase(unittest.TestCase):
    def test_hmc_dd_process_data(self):
        ...
        training_data, basis_data = hmc_dd.process_training_data(results_burnin[3], results_burnin[4],
                                                                 results_burnin[5],
                                                                 mass_mat=mass_mat, num_sol_basis=num_sol_basis,
                                                                 num_grad_basis=num_grad_basis)
        # check input data
        ...
        self.assertEqual(
            np.allclose(training_data['x'] * basis_data['std_x'] + basis_data['mean_x'], results_burnin[3]), True)
        # check solution output
        self.assertEqual(np.allclose(np.diag(basis_data['phi_sol'].transpose() @ mass_mat @ basis_data['phi_sol']), 1),
                         True)
        ...
        project_diff = basis_data['phi_sol'] @ (
                    training_data['y_sol'] * basis_data['std_y_sol'] + basis_data['mean_y_sol']) + basis_data[
                           'mean_sol'] - results_burnin[4]
        rel_err = np.sqrt(
            np.diag((project_diff.T @ mass_mat @ project_diff) / (results_burnin[4].T @ mass_mat @ results_burnin[4])))
        print('For solution space, %d basis with mean, L^2 error mean: %.6f std: %.6f' % (
        num_sol_basis, rel_err.mean(), rel_err.std()))
        self.assertEqual(True, True)
        for i in range(num_kl):
            self.assertEqual(np.allclose(
                np.diag(basis_data['phi_grad'][:, i, :].transpose() @ mass_mat @ basis_data['phi_grad'][:, i, :]), 1),
                             True)
            ...
            project_diff = basis_data['mean_grad'][:, i, :] + basis_data['phi_grad'][:, i, :] @ (
                        training_data['y_grad'][:, i, :] * basis_data['std_y_grad'][:, i, :] + basis_data[
                                                                                                   'mean_y_grad'][:, i,
                                                                                               :]) - results_burnin[5][
                                                                                                     :, i, :]
            rel_err = np.sqrt(np.diag((project_diff.T @ mass_mat @ project_diff) / (
                        results_burnin[5][:, i, :].transpose() @ mass_mat @ results_burnin[5][:, i, :])))
            print('For gradient space, %d-th partial derivative, %d basis with mean, L^2 error mean: %.6f std: %.6f' % (
            i, num_grad_basis, rel_err.mean(), rel_err.std()))

        # test training process
        net = hmc_dd.SolutionMap(size_in=num_kl, size_out=num_sol_basis, size_hidden=5)
        net.double()
        net, train_time, train_loss, dev_loss = hmc_dd.trainit(net, training_data['x'], training_data['y_sol'],
                                                               opt='SGD', epochs=200, lr=0.02, num_iter=5)
        net_grad = []
        for i in range(num_kl):
            net1 = hmc_dd.GradientMap(size_in=num_kl, size_out=num_grad_basis, size_hidden=20)
            net1.double()
            net1, _, _, _ = hmc_dd.trainit(net1, training_data['x'], training_data['y_grad'][:, i, :], opt='Adam',
                                           epochs=200, lr=0.02, num_iter=5)
            net_grad.append(net1)

        # test solve via neural network
        u, pu = hmc_dd.solve_via_data_driven(np.random.randn(num_kl), net, dataset=basis_data, derivative=1,
                                             net_grad=net_grad)
        self.assertEqual(u.shape, ((N + 1) ** 2, 1))
        self.assertEqual(pu.shape, ((N + 1) ** 2, num_kl))
        # test compute potential
        theta = np.random.randn(num_kl)
        pot = hmc_dd.compute_potential_via_data_driven(theta, net, net_grad, hmc_inv_pde, basis_data, order=0)
        print('potential = ', pot)
        dpot = hmc_dd.compute_potential_via_data_driven(theta, net, net_grad, hmc_inv_pde, basis_data, order=1)
        print('potential gradient = ', dpot)

        theta_samples, acp, timer = hmc_dd.hmc_evolve(hmc_inv_pde=hmc_inv_pde, num_of_iter=total_iter_num,
                                                      start_theta=results_burnin[0], net=net,
                                                      net_grad=net_grad, basis_data=basis_data, step_size=step_size,
